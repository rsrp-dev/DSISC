...
//
var portionOfScreen = 0.33 // Small radar will take up this much of the screen in %. Default: 0.33
...
var radarColorPlane = "#89F6FF" // Sets the color of airplane and helicopter radar entries. Default: #89F6FF
...
var circleDashSize = 4 // Adjusts the appearance of the dashed circle on the radar/sonar screen. Default: 4
var circleDashSpace = 9.5 // Adjusts the appearance of the dashed circle on the radar/sonar screen. Default: 9.5
var infoTextFont = "18px Courier" // Adjusts the appearance of the range & frequency text in the bottom left corner of the radar screen. Default: "18px Courier"
...
function drawRadarTarget(deltaX, deltaY, deltaZ, name, range, speed, color) {
	...
	drawText(name, x+10, y-20, null, color)
...
function drawInfoText(startTime, debugText) { //TODO clean this up, use measureText
	...
	drawText(radarName.toTitleCase() + " Range: " + radarRange + " m", 10, canvas.height - 27, infoTextFont)
	drawText("Refresh Rate: " + getHz() + " Hz", 10, canvas.height - 10, infoTextFont)
	if (debugText || false) {
...
function updateRadarInfo(range, freq, _yaw, _radarType, _radarName) {
	radarRange = range
...
window.addEventListener('message', function(event) {
	...
	if (item.command == 'setRadarOpen') {
		...
		updateRadarInfo(item.range, item.freq, item.yaw, item.radarType, item.radarName)			
	}
