...
local radioButton				= 82 --- U by default  -- use 57 for f10
local handsUpButton				= 217 --- H by default -- use 73 for X
	...
	if tostring(args[1]) == nil then
		...
		if tostring(args[1]) ~= nil then
			...
			elseif argh == 'notes' then
				...
				-- Animations With Mobile Phone! YEET
			...
			elseif argh == 'textphone' then
				...
				if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
					loadAnimDict( ad )
					if ( IsEntityPlayingAnim( player, ad, "cellphone_cellphone_text_base", 3 ) ) then 
						TaskPlayAnim( player, ad, "exit", 8.0, 1.0, -1, 49, 0, 0, 0, 0 )
						Wait (100)
						ClearPedSecondaryTask(PlayerPedId())
						DetachEntity(prop, 1, 1)
						...
						local x,y,z = table.unpack(GetEntityCoords(player))
						prop = CreateObject(GetHashKey(prop_name), x, y, z+0.2,  true,  true, true)
						AttachEntityToEntity(prop, player, GetPedBoneIndex(player, 57005), 0.15, 0.02, -0.01, 50.0, 35.0, 140.0, true, true, false, true, 1, true)
						TaskPlayAnim( player, ad, "cellphone_cellphone_text_base", 8.0, 1.0, -1, 49, 0, 0, 0, 0 )
					end 
				end
