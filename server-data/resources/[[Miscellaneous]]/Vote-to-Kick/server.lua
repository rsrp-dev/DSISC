...
RegisterCommand(voteCmd, function(source, args, rawCommand)
    ...
    if canStartVote == true then
        if tID then 
        	if GetPlayerName(args[1]) and IsPlayerAceAllowed(tID, "vMenu.DontKickMe") then
        		DropPlayer(source, "Nice try <3")
            elseif GetPlayerName(args[1]) and not IsPlayerAceAllowed(tID, "vMenu.DontKickMe") then
...
RegisterCommand(CancelCmd, function(source, args, rawCommand)
    if faxCore then
        ...
        if IsPlayerAceAllowed(source, "vMenu.DontKickMe") then
            voteActive = false;voteCount = 0;canStartVote = false
            svrSeconds = 0;svrMinutes = 0
            TriggerClientEvent("chatMessage", -1, "^*^1VOTE TO KICK ^7was canceled by an admin")
        else
